...
int main() {
	...
	Shader shader("src/shaders/transform.vert", "src/shaders/transform.frag");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// glm::value_ptr converts data to GL readable format
		// Matrices are column major ordered by default
		glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(trans));

		glBindVertexArray(VAO);
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);

		// Draw a second rectangle and transform it
